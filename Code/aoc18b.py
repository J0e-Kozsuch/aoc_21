...
for index1, number1 in enumerate(list_lines):
    for index2, number2 in enumerate(list_lines):
        
        if index1 != index2:
            tree1 = list_node(number1, 0, None, None)
            tree1.create_children_nodes()

            tree2 = list_node(number2, 0, None, None)
            tree2.create_children_nodes()


            master_tree = add_numbers(tree1, tree2)

            reduce(master_tree)
            
            tree_sum = master_tree.calculate_magnitude()

            if tree_sum > highest_sum:
                highest_sum = tree_sum
